...
const getResetSession = `-- name: GetResetSession :one
SELECT session_id, created_at, updated_at, user_id, last_login, ip_addr, user_agent, expired_at, is_reset_session FROM login_session
WHERE session_id=$1 AND ip_addr=$2 AND user_agent=$3 AND is_reset_session IS TRUE
`

type GetResetSessionParams struct {
	SessionID string
	IpAddr    pqtype.Inet
	UserAgent sql.NullString
}

func (q *Queries) GetResetSession(ctx context.Context, arg GetResetSessionParams) (LoginSession, error) {
	row := q.db.QueryRowContext(ctx, getResetSession, arg.SessionID, arg.IpAddr, arg.UserAgent)
	var i LoginSession
	err := row.Scan(
		&i.SessionID,
		&i.CreatedAt,
